...
int Calculte_velocity_to_PWM(float input_velocity)
{
  float distance_per_once_wheel_move = 0;
  float velocity_per_minute = 0; //m/m
  int output_pwm = 0;

  distance_per_once_wheel_move = 2*3.141592*WHEEL_RADIUS; //mm
  velocity_per_minute = (MAX_RPM*distance_per_once_wheel_move)/1000; //m/m
  
  output_pwm = (input_velocity*MAX_RPM)/velocity_per_minute;
...
void MOTOR (const geometry_msgs::Twist& msg)
{
  ...
  move_pwm = Calculte_velocity_to_PWM(VELOCITY);
  ...
  if(encoder_msg.angular.z < move_tick)
  {
    ...
    analogWrite(ENABLE_A, move_pwm);
    analogWrite(ENABLE_B, move_pwm);
  }
